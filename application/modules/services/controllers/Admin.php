...
class Admin extends Admin_Controller {
    ...
    function record($cipher='null') {
           ...
           /* echo "<pre>";
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(  $this->Mservices->Save() ){
                    ...
                    update_dop_user_log('has added '.$this->Mservices->table_name);
                }
                else {
                    $this->session->set_flashdata('_flash_msg', array(
                        '_css_cls'  => 'error',
                ...
                intval($this->Encryption->decode($cipher))?redirect('admin/'.$this->Mservices->table_name, 'refresh'):
                redirect('admin/'.$this->Mservices->table_name.'/record/list', 'refresh');
                exit();
            }
        }
        if(isset($cipher) && intval($this->Encryption->decode($cipher))) {
            $data_content   =  array(
                'page_header'   => 'Edit Services',
                'record'        => $this->Mservices->PickRecord($cipher)
            );
        ...
        elseif(isset($cipher) && $cipher=="list") {
            $data_content=array(
                'page_header'   => 'List of Services',
                'records'     => $this->Mservices->PickRecord()
            );
