...
class Admin extends Admin_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('Mreports');
         $this->resource_path = realpath( APPPATH . '../uploads/reports/' );
        $this->resource_path_url  = base_url() . 'uploads/reports/';
        $this->_image_config = array(
                        'allowed_types' => 'gif|png|jpeg|jpg|pdf',
    ...
    function getFiscalYearData(){
        $query = $this->db->query("select * from fiscal_year where status = 1");
        return $query->result();
    ...
    function record($cipher='null') { 
        ...
        {  
            ...
            if(  $this->Mreports->Save() )
            {
                ...
                update_dop_user_log('has added '.$this->Mreports->table_name); 
            }
            else
            {
                $this->session->set_flashdata('_flash_msg', array(
            ...
            intval($this->Encryption->decode($cipher))?redirect('admin/'.$this->Mreports->table_name, 'refresh'):
            redirect('admin/'.$this->Mreports->table_name.'/record/list', 'refresh');
            exit();
        }
        }
        if(isset($cipher) && intval($this->Encryption->decode($cipher))) {					
            $data_content   =  array(
                ...
                'record'        => $this->Mreports->PickRecord($cipher),
        ...
        elseif(isset($cipher) && $cipher=="list") {
            $data=array(
                           'records'     => $this->Mreports->PickRecord(),
                           'fiscal'    => $this->getFiscalYearData(),
            );
  ...
  function delete($id){
   ...
   function delete_file($fullpath)
      ...
      function delete_reports_file(){
        $record=$this->Mreports->get(intval($this->input->post('file_id')));
        $this->delete_file($this->resource_path . '/' . $record->file);
           if($this->db->query("update reports set file=null where id = '".$this->input->post('file_id')."'"))               
        {
