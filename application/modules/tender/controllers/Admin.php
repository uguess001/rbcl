...
class Admin extends Admin_Controller {
...

	{
		...
		$this->load->model('Mtender');

         $this->resource_path = realpath( APPPATH . '../uploads/tender/' );

        $this->resource_path_url  = base_url() . 'uploads/tender/';
    ...
    function record($cipher='null') { 
        ...
        {  
...

            {
                ...
                update_dop_user_log('has added '.$this->Mtender->table_name); 
            ...
            intval($this->Encryption->decode($cipher))?redirect('admin/'.$this->Mtender->table_name, 'refresh'):

            redirect('admin/'.$this->Mtender->table_name.'/record/list', 'refresh');
        ...
        if(isset($cipher) && intval($this->Encryption->decode($cipher))) {					
            ...
            $data_content   =  array(
                ...
                'record'        => $this->Mtender->PickRecord($cipher)
        ...
        elseif(isset($cipher) && $cipher=="list") {
            ...
            $data=array(
                           ...
                           'records'     => $this->Mtender->PickRecord()
  ...
  function delete($id){
   ...
   function delete_file($fullpath)
      ...
      function delete_tender_file(){

         $record=$this->Mtender->get(intval($this->input->post('file_id')));

        $this->delete_file($this->resource_path . '/' . $record->file);

           if($this->db->query("update tender set file=null where id = '".$this->input->post('file_id')."'"))               
