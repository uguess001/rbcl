...
class Admin extends Admin_Controller {
    function __construct(){
        ...
        $this->resource_path = realpath( APPPATH . '../uploads/payments/' );
        $this->resource_path_url  = base_url() . 'uploads/payments/';
    ...
    function record($cipher='null') {
        ...
        if( $this->input->post('process') == 'true' ){
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(  $this->Mpayment->Save() ){
                    ...
                    update_dop_user_log('has added '.$this->Mpayment->table_name);
                }
                else {
                    $this->session->set_flashdata('_flash_msg', array(
                        '_css_cls'  => 'error',
                ...
                intval($this->Encryption->decode($cipher))?redirect('admin/'.$this->Mpayment->table_name, 'refresh'):
                redirect('admin/'.$this->Mpayment->table_name.'/record/list', 'refresh');
                exit();
            }
        }
        if(isset($cipher) && intval($this->Encryption->decode($cipher))) {
            $data_content   =  array(
                'page_header'   => 'Edit payments',
                'record'        => $this->Mpayment->PickRecord($cipher)
            );
        ...
        elseif(isset($cipher) && $cipher=="list") {
            $data_content=array(
                'page_header'   => 'List of payments',
                'records'     => $this->Mpayment->PickRecord()
            );
