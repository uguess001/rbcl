...
class Admin extends Admin_Controller {
	...
	function __construct()
	{
		parent::__construct();
		parent::index();
		$this->load->model('Mtestimonials');
         $this->resource_path = realpath( APPPATH . '../uploads/testimonials/' );
        $this->resource_path_url  = base_url() . 'uploads/testimonials/';
    ...
    function record($cipher='null') { 
        ...
        {  
            ...
            if(  $this->Mtestimonials->Save() )
            {
                ...
                update_dop_user_log('has added '.$this->Mtestimonials->table_name); 
            }
            else
            {
                $this->session->set_flashdata('_flash_msg', array(
            ...
            intval($this->Encryption->decode($cipher))?redirect('admin/'.$this->Mtestimonials->table_name, 'refresh'):
            redirect('admin/'.$this->Mtestimonials->table_name.'/record/list', 'refresh');
            exit();
        }
        }
        if(isset($cipher) && intval($this->Encryption->decode($cipher))) {					
            $data_content   =  array(
                ...
                'record'        => $this->Mtestimonials->PickRecord($cipher)
                );
        ...
        elseif(isset($cipher) && $cipher=="list") {
            $data=array(
                           'records'     => $this->Mtestimonials->PickRecord()
            );
