...
class Admin extends Admin_Controller {
...

	{
        ...
        //loading the data of Downloads Category inside the select field of downloads.
        $this->load->model('downloadscategory/Mcategory');

         $this->resource_path = realpath( APPPATH . '../uploads/downloads/' );

        $this->resource_path_url  = base_url() . 'uploads/downloads/';
    ...
    function record($cipher='null') { 
        ...
        {  
...

            {
                ...
                update_dop_user_log('has added '.$this->Mdownloads->table_name); 
            ...
            intval($this->Encryption->decode($cipher))?redirect('admin/'.$this->Mdownloads->table_name, 'refresh'):

            redirect('admin/'.$this->Mdownloads->table_name.'/record/list', 'refresh');
        ...
        if(isset($cipher) && intval($this->Encryption->decode($cipher))) {					
            ...
            $data_content   =  array(
                ...
                'record'        => $this->Mdownloads->PickRecord($cipher),
                
                'category'   => $this->Mcategory->PickRecord(),

                'fiscal'    => $this->getFiscalYearData(),
        ...
        elseif(isset($cipher) && $cipher=="list") {
            ...
            $data=array(
                   ...
                   'records'     => $this->Mdownloads->PickRecord(),
                   'category'   => $this->Mcategory->PickRecord(),
                   'fiscal'    => $this->getFiscalYearData(),
  ...
  function delete($id){
   ...
   function delete_downloads_file(){

    $record=$this->Mdownloads->get(intval($this->input->post('file_id')));

    $this->delete_file($this->resource_path . '/' . $record->file);

           if($this->db->query("update downloads set file=null where id = '".$this->input->post('file_id')."'"))               
