...
class Mweensure extends MY_Model{
    private $data = null;
...
$new_path=$this->input->post('image');
...
die;*/
    ...
    $this->data   = array(
        'slug'         =>$slug,
        'icon'         =>$this->input->post('cd-dropdown'),
        ...
        'email'     => ($this->input->post('email'))?$this->input->post('email'):'info@rbcl.com.np',
        'status'     => $this->input->post('status'),
        'published_date'     => $this->input->post('published_date'),
        'showonfooter'      => $this->input->post('showonfooter') ? $this->input->post('showonfooter') : 0,
    );
}
...
function get_record(){
    $data=$this->db->query("select * from weensure where status=1")->result();
    return $data;
}
}